...
class TestChatbotUI:
    ...
    def test_open_chat_panel(self, chatbot_page: ChatbotPage):
        ...
        assert not chatbot_page.is_chat_panel_visible(), "El panel de chat debería estar cerrado inicialmente"
        
        # Abrir el chat
        chatbot_page.open_chat()
        assert chatbot_page.is_chat_panel_visible(), "El panel de chat debería estar visible después de abrirlo"
    ...
    def test_close_chat_panel(self, chatbot_page: ChatbotPage):
        ...
        assert not chatbot_page.is_chat_panel_visible(), "El panel de chat debería estar cerrado después de cerrarlo"
    ...
    def test_send_button_functionality(self, chatbot_page: ChatbotPage):
        ...
        assert chatbot_page.is_send_button_enabled(), "El botón de enviar debería estar habilitado"
        ...
        assert test_message in user_messages, "El mensaje del usuario debería aparecer en el chat"
    ...
    def test_enter_key_send(self, chatbot_page: ChatbotPage):
        ...
        user_messages = chatbot_page.get_all_user_messages()
        assert test_message in user_messages, "El mensaje enviado con Enter debería aparecer en el chat"
    ...
    def test_reset_chat_for_specific_test(self, chatbot_page: ChatbotPage):
        ...
        assert not chatbot_page.is_chat_panel_visible(), "El panel de chat debería estar cerrado después del reinicio"
        ...
        user_messages = chatbot_page.get_all_user_messages()
        assert len(user_messages) == 1, "Debería haber solo un mensaje después de reiniciar"
        assert "Mensaje después del reinicio" in user_messages, "El mensaje después del reinicio debería estar en el chat"
    ...
    def test_multiple_messages_conversation(self, chatbot_page: ChatbotPage):
        ...
        user_messages = chatbot_page.get_all_user_messages()
        for message in messages:
            assert message in user_messages, f"El mensaje '{message}' debería estar en el chat"
        
        # Verificar que hay respuestas del bot para cada mensaje
        bot_messages = chatbot_page.get_all_bot_messages()
        assert len(bot_messages) >= len(messages), "Debería haber al menos una respuesta del bot por cada mensaje del usuario"
