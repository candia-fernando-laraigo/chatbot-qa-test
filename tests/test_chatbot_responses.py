...
def test_greeting_responses(driver, chatbot_page, greeting):
    ...
    assert greeting in chatbot_page.get_all_user_messages(), "User message not displayed in chat"
    ...
    assert any(word in bot_response.lower() for word in [
        "hola", "bienvenido", "saludos", "ayudar"
...
def test_price_inquiry_responses(driver, chatbot_page, query):
    ...
    assert query in chatbot_page.get_all_user_messages(), "User message not displayed in chat"
    bot_response = chatbot_page.wait_for_bot_response()
    response_time = time.time() - start_time
    ...
    assert any(word in bot_response.lower() for word in [
        "precio", "costo", "valor", "plan", "paquete", "ventas"
...
def test_product_service_info_responses(driver, chatbot_page, query):
    ...
    assert any(word in bot_response.lower() for word in [
        "servicio", "producto", "ofrecemos", "plataforma", "solución"
...
def test_contact_info_responses(driver, chatbot_page, query):
    ...
    assert any(word in bot_response.lower() for word in [
        "contacto", "email", "correo", "teléfono", "llamar", "comunicarse"
